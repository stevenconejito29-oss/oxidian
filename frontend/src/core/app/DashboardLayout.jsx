/**
 * DashboardLayout â€” Sidebar + topbar para paneles admin/tenant.
 * Se adapta al rol del usuario y colapsa en mÃ³vil.
 */
...
const Icon = ({ name, size = 18 }) => {
  const icons = {
    ...
    combos: 'M7 2h10l5 5v10l-5 5H7l-5-5V7l5-5zm1 5v10h8V7H8zm2 2h4v2h-4V9zm0 4h4v2h-4v-2z',
    inventory: 'M3 3h18v4H3V3zm2 6h14v12H5V9zm3 3v2h8v-2H8zm0 4v2h5v-2H8z',
    finance: 'M12 1l3 6 6 .87-4.5 4.39 1.06 6.21L12 15.77 6.44 18.47 7.5 12.26 3 7.87 9 7l3-6zm-1 8v6h2V9h-2z',
  }
  return (
...
const BRANCH_ADMIN_NAV = [
  ...
  { to: '/branch/admin#staff', icon: 'users', label: 'Staff', tab: 'staff' },
  { to: '/branch/admin#marketing', icon: 'affiliate', label: 'Marketing', tab: 'marketing' },
  { to: '/branch/admin#finance', icon: 'finance', label: 'Caja', tab: 'finance' },
  { to: '/branch/admin#chatbot', icon: 'chatbot', label: 'Chatbot', tab: 'chatbot' },
...
const NAV_BY_ROLE = {
  ...
  tenant_owner: [
    { to: '/tenant/admin', icon: 'overview', label: 'Panel', tab: 'overview' },
    ...
    { to: '/tenant/admin#customize', icon: 'settings', label: 'DiseÃ±o', tab: 'customize' },
  ],
  tenant_admin: [
    { to: '/tenant/admin', icon: 'overview', label: 'Panel', tab: 'overview' },
    { to: '/tenant/admin#stores', icon: 'stores', label: 'Tiendas', tab: 'stores' },
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:'var(--color-background-secondary)' }}>
      ...
      <aside style={{
        ...
        <div style={{
          ...
          <button onClick={() => setCollapsed(value => !value)} style={{
        ...
        <nav style={{ flex:1, padding:'8px 8px', overflow:'auto' }}>
          {navItems.map(item => {
            const isActive = activeTab ? item.tab === activeTab : item.tab === navItems[0]?.tab
            return (
              <button
                key={item.to}
