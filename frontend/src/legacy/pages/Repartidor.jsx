...
import RepartidorContent from './RepartidorContent'
...
function loadSession() {
  try {
    ...
    if (data.role && data.role !== 'repartidor') {
      sendStaffOfflineBeacon(data.id, data.store_id)
      localStorage.removeItem(SESSION_KEY)
      return null
    }
...
export default function Repartidor() {
  ...
  return (
    <StaffPWARequired role="repartidor">
      {!session ? (
        <StaffLogin role="repartidor" onLogin={handleLogin} />
      ) : (
        <RepartidorContent session={session} onLogout={logout} />
      )}
    </StaffPWARequired>
  )
}
